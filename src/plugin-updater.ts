/** 复用同系列 Automation 的 npm 更新流程，适配宠物 Host 路由。 */
...
export type HostRequest = { method?: string; url?: string; headers?: Record<string, string | string[] | undefined>; socket?: { remoteAddress?: string } }
export type HostResponse = { writeHead(status: number, headers?: Record<string, string>): void; end(body?: string): void }
...
export type UpdateContext = { webServer: WebServer; get?(name: string): unknown; logger: { warn(message: string): void } }
...
async function latestVersion(packageName: string): Promise<string | null | undefined> {
  ...
  try {
    ...
    if (typeof value.version !== 'string' || !parseSemver(value.version)) return undefined
...
async function currentVersion(manifestUrl: URL): Promise<string> {
  ...
  if (typeof value.version !== 'string' || !parseSemver(value.version)) throw new Error('无法读取当前插件版本。')
  return value.version
}
...
export function registerPluginUpdater(ctx: UpdateContext, options: PluginUpdaterOptions): () => void {
