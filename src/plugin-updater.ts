import { UpdateFailure, updateErrorPayload } from "./update-errors.ts";
...
export function isDshCliEntry(
  ...
  return (
    ...
    bin !== "" &&
    !isAbsolute(bin) &&
    resolve(packageRoot, bin) === resolve(entry)
  );
}
...
let latestCache:
  | { packageName: string; version: string; expiresAt: number }
  | undefined;
...
async function currentVersion(manifestUrl: URL): Promise<string> {
  const value = JSON.parse(await readFile(manifestUrl, "utf8")) as {
    version?: unknown;
  };
  if (typeof value.version !== "string" || !parseSemver(value.version))
    ...
    handler: async (request, response) => {
      try {
        ...
        if (request.method === "GET" || request.method === "HEAD") {
          ...
          response.end(
            request.method === "HEAD" ? undefined : JSON.stringify(payload),
          );
          return;
        }
