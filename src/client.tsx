...
interface ClientContext { sessions: Sessions & CreationSessions; uiSession: { pendingInteractions: PendingStore }; slots: { inject(name: string, register: () => () => void): void; register(options: { name: string; id: string; order?: number; label?: string }, component: () => React.ReactElement): () => void } }
...
function Overlay({ sessions, pending }: { sessions: Sessions & CreationSessions; pending: PendingStore }) {
...
function Page({ sessions }: { sessions: CreationSessions }) { const controller = usePetController(); return <Settings controller={controller} create={async description => { const library = controller.library; if (!library) throw new Error('宠物库尚未加载'); await createPetSession(sessions, description, library.customPath, library.skillPath); }} />; }
export function apply(ctx: ClientContext): void {
  ctx.slots.inject('settings.section', () => ctx.slots.register({ name: 'settings.section', id: 'codex-pet', label: '宠物', order: 12 }, () => <Page sessions={ctx.sessions} />));
  ctx.slots.inject('shell.overlay', () => ctx.slots.register({ name: 'shell.overlay', id: 'michengai-codex-pet', order: 100 }, () => <Overlay sessions={ctx.sessions} pending={ctx.uiSession.pendingInteractions} />));
}
