...
interface ClientContext { locale: PetLocaleStore; sessions: Sessions & CreationSessions; uiSession: { pendingInteractions: PendingStore }; slots: { inject(name: string, register: () => () => void): void; register(options: { name: string; id: string; order?: number; label?: string | (() => string) }, component: () => React.ReactElement): () => void } }
...
function openSettings(): void {
  ...
  if (trigger) trigger.dispatchEvent(new CustomEvent('dcu-settings-open-section', { bubbles: true, cancelable: true, detail: { labels: ['宠物', 'Pets'] } }));
...
export function apply(ctx: ClientContext): void {
  ctx.slots.inject('settings.section', () => ctx.slots.register({ name: 'settings.section', id: 'codex-pet', label: () => translator(ctx.locale.getSnapshot().active)('宠物'), order: 12 }, () => <Page sessions={ctx.sessions} locale={ctx.locale} />));
